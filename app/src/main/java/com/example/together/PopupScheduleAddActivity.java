...
public class PopupScheduleAddActivity extends AppCompatActivity {
    ...
    //속성(변수) 선언
    private static String TAG = "PopupScheduleAddActivity";
    ...
    EditText scheduleLocation; //일정 장소 입력란
    Button searchButton; //지도검색 버튼
...
//        supportRequestWindowFeature(Window.FEATURE_NO_TITLE); // 상단 타이틀 제거
        ...
        scheduleSelectDate.setText(getToday("yyyy.MM.dd"));
...
//                Log.e(TAG, String.valueOf(mYear));
        ...
        scheduleSelectTime.setText(getToday("HH:mm"));
        ...
        searchButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(PopupScheduleAddActivity.this, SearchLocationActivity.class);
...
//                Log.e(TAG, String.valueOf(timePicker.getCurrentMinute()));
                ...
                if (scheduleTitle.getText().toString().length() == 0) {
                    Toast.makeText(PopupScheduleAddActivity.this, "제목을 입력하세요.", Toast.LENGTH_SHORT).show();
                    return;
                } else if (scheduleContent.getText().toString().length() == 0) {
                    Toast.makeText(PopupScheduleAddActivity.this, "내용을 입력하세요.", Toast.LENGTH_SHORT).show();
                    return;
                } else if (scheduleLocation.getText().toString().length() == 0) {
                    Toast.makeText(PopupScheduleAddActivity.this, "장소를 입력하세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                //제목, 내용, 날짜, 시간(오전/오후), 시간(시), 시간(분), 장소, 모임 idex 순서
                task.execute("http://" + IP_ADDRESS + "/group/schedule_add.php", scTitle, scContent, scDate, scClock, scTime, scMinute, scLocation, String.valueOf(group));
    ...
    class InsertData extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            ...
            progressDialog = ProgressDialog.show(PopupScheduleAddActivity.this,
...
//            mTextViewResult.setText(result);
        ...
        protected String doInBackground(String... params) {
            ...
            //제목, 내용, 날짜, 시간(오전/오후), 시간(시), 시간(분), 장소, 모임 idex 순서
            String postParameters = "scTitle=" + scTitle + "&scContent=" + scContent + "&scDate=" + scDate + "&scClock=" + scClock + "&scTime=" + scTime + "&scMinute=" + scMinute + "&scLocation=" + scLocation + "&group=" + group;
...
// MainActivity 에서 요청할 때 보낸 요청 코드 (3000)
                case 3000:
                    // 장소란에 결과값 입력
                    scheduleLocation.setText(data.getStringExtra("locationResult"));
                    break;
            }
        }
    }
